...
import { AppBar, Button, CardMedia, Toolbar, Typography, Card, CardContent } from '@material-ui/core'
...
const useStyles = makeStyles({
    ...
    cardContent: {
        position: "absolute",
        color: "#FFFFFF",
        marginTop: window.innerHeight * 0.50,
    }
})
...
const Browse: FunctionComponent< { history: History } > = ( { history } ) => {
    ...
    useEffect( () => {
        const getPrincipal = async () => {
            const fetchMovies = await ( await fetch( `${baseUrl}latest?api_key=${token}${extra}` ) ).json()
        ...
        <div style = { { backgroundColor: "#303030" } } >
            <AppBar className = { classes.appBar } >
                <Toolbar variant = "dense" className = { classes.toolBar } >
                    ...
                    <Button className = { classes.button } variant = "contained" onClick = { handleClick } > Log out </Button>
                </Toolbar>
            </AppBar>
            {/* <MovieRow url = { `${baseUrl}latest?api_key=${token}${extra}` } title = "Lasted" /> */}
            <Card className = { classes.principal } >
                <CardMedia image = { principalMovie.poster_path != null ? `${imageUrl}${principalMovie.poster_path}` : `${process.env.PUBLIC_URL}/assets/no-image.png` } className = { classes.media } />
                <CardContent className = { classes.cardContent } >
                    <Typography variant = "h2" style = { { textShadow: "1px 2px #000000", } } >{principalMovie.original_title}</Typography>
